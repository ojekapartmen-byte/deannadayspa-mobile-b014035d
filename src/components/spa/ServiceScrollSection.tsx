  ...
  return (
    <section className="py-12 w-full">
      ...
      <div className="px-4 mb-10">
        ...
        <div className="h-1.5 w-24 bg-primary/20 mt-4 rounded-full overflow-hidden">
      ...
      <div 
        className="flex gap-6 overflow-x-auto snap-x snap-mandatory px-4 pb-10 scrollbar-hide md:grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 md:overflow-visible md:snap-none"
      ...
      >
        {services.map((service, i) => (
          <motion.div
            ...
            className="min-w-[300px] w-[85vw] max-w-[350px] snap-center shrink-0 rounded-[2.5rem] overflow-hidden shadow-2xl relative h-[480px] md:min-w-0 md:w-full md:max-w-none group bg-card"
          >
            ...
            <div className="relative z-10 p-7 h-full flex flex-col justify-end">
              ...
              {/* Product Info */}
              <div className="mb-6">
                <h3 className="font-display text-2xl md:text-3xl text-white font-bold leading-tight drop-shadow-md">
                  {service.title}
                </h3>
                <p className="text-[12px] text-white/60 font-body italic mt-2 line-clamp-2 leading-relaxed">
                  {service.description}
                </p>
              </div>
              ...
              <div className="space-y-3">
                {service.service_options.map((opt) => (
                  <div
                    key={opt.id}
                    className="flex items-center p-4 rounded-[1.5rem] backdrop-blur-xl bg-white/5 border border-white/10 hover:bg-white/15 transition-all duration-300 min-h-[75px]"
                  >
                    {/* Text Area - Sejajar Vertikal */}
                    <div className="flex-grow flex flex-col justify-center pr-3">
                      <p className="text-[10px] font-bold text-primary tracking-[0.2em] uppercase mb-1 opacity-90">
                        {opt.name}
                      </p>
                    ...
                    {/* WhatsApp Icon Button - Sejajar Horizontal */}
                    <a
                      href={getBookingUrl(opt.booking_text, waNumber)}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex-shrink-0 flex items-center justify-center w-11 h-11 bg-primary text-white rounded-full shadow-lg transition-all hover:scale-110 active:scale-90 hover:rotate-12 group/btn shadow-primary/20"
