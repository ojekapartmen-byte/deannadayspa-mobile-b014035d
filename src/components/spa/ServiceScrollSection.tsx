...
const ServiceScrollSection = ({ title, highlight, services, isLoading, waNumber = "6281999231518" }: Props) => {
  ...
  return (
    <section className="py-12 w-full">
      ...
      {/* Container: Horizontal Scroll di Mobile, Grid Rapih di Desktop */}
      <div 
      ...
      >
        {services.map((service, i) => (
          <motion.div
            ...
            className="min-w-[300px] w-[85vw] max-w-[350px] snap-center shrink-0 rounded-[2.5rem] overflow-hidden shadow-2xl relative h-[450px] md:min-w-0 md:w-full md:max-w-none group bg-card"
          >
            {/* Image Layer dengan Zoom halus saat Hover */}
            <img
            ...
            <div className="relative z-10 p-7 h-full flex flex-col justify-end">
              ...
              {/* Glassmorphism Options List */}
              <div className="space-y-3">
                {service.service_options.map((opt) => (
                  <div
                    key={opt.id}
                    className="flex items-center justify-between p-4 rounded-[1.5rem] backdrop-blur-xl bg-white/5 border border-white/10 hover:bg-white/15 transition-all duration-300 group/item"
                  >
                    <div className="flex-grow">
                      <p className="text-[10px] font-bold text-primary tracking-[0.2em] uppercase mb-0.5">
                        {opt.name}
                      </p>
                    ...
                    {/* WhatsApp Icon Button Minimalis */}
                    <a
                      href={getBookingUrl(opt.booking_text, waNumber)}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex items-center justify-center w-11 h-11 bg-primary text-white rounded-full shadow-lg transition-all hover:scale-110 active:scale-90 hover:rotate-12 group-hover/item:shadow-primary/50"
