...
const ServiceSection = ({ title, highlight, services, isLoading, waNumber = "6281999231518" }: Props) => {
  ...
  return (
    <section className="py-10 px-4">
      <h2 className="font-display text-3xl md:text-4xl text-foreground mb-8 px-1">
        {title}{" "}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {services.map((service, i) => (
          <motion.div
            ...
            className="rounded-3xl overflow-hidden shadow-xl relative min-h-[320px] group"
          >
            ...
            <div className="relative z-10 p-5 h-full flex flex-col justify-end">
              <div className="mb-4">
                <h3 className="font-display text-2xl text-white font-bold leading-tight mb-1">
                  {service.title}
                </h3>
                <p className="text-xs text-white/80 font-body italic line-clamp-2">
                  {service.description}
                </p>
              </div>
              ...
              <div className="space-y-3">
                {service.service_options.map((opt) => (
                  <div
                    key={opt.id}
                    className="flex items-center justify-between p-3.5 rounded-2xl backdrop-blur-lg bg-white/10 border border-white/20 hover:bg-white/20 transition-colors"
                  >
                    ...
                    <a
                      ...
                      className="flex items-center justify-center w-11 h-11 bg-primary text-primary-foreground rounded-full transition-all active:scale-90 shadow-lg hover:rotate-12"
